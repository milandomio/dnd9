...
- **候选总数**：130 个实体。
- **分类**：88 个已有 `df5.hardcoded.*` synthetic key 但日语值仍等于英文；42 个 `translation_key` 为空，页面使用原始英文标识或中文兜底。
- **数据依据**：`data/json/{items,monsters,props,lootdrops,dungeon_modules.json,locale/{ja,en}.json}` 与已生成的 `dist/ja` 页面。
