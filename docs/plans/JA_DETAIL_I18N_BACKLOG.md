...
- **候选总数**：0 个未本地化标题。
- **分类**：所有详情实体已具备官方或 `df5.hardcoded.*` key；五个怪物和五个场景实体已补十语言人工词条，`LittleToad_Poison`、`LivingArmor`、`LivingStatue`、`Morayeel`、`Rat`、`TrainingDummy_CharacterBase` 和 `Ruins_Chapel` 复用官方 key；其余技术实体使用十语言“技术对象 + 可读资产名”自动词条，后续仅做文案润色。
