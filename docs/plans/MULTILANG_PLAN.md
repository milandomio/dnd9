# 多语言文档索引

原始多语言重构计划已完成大部分落地，完整设计、阶段记录和历史问题保存在 [`MULTILANG_PLAN_ARCHIVE.md`](MULTILANG_PLAN_ARCHIVE.md)。日常按主题阅读以下小文档，避免每次加载 600 行计划全文。

## 当前入口

| 主题 | 文档 |
|------|------|
| 路由、翻译键、locale、Provider、组件边界 | [`MULTILANG_ARCHITECTURE.md`](MULTILANG_ARCHITECTURE.md) |
| 管道、SSG、sitemap、PWA、部署和验收 | [`MULTILANG_BUILD_AND_TEST.md`](MULTILANG_BUILD_AND_TEST.md) |
| P0-P11 状态、已知问题和后续事项 | [`MULTILANG_STATUS.md`](MULTILANG_STATUS.md) |
| 完整原计划及历史回写 | [`MULTILANG_PLAN_ARCHIVE.md`](MULTILANG_PLAN_ARCHIVE.md) |

## 维护规则

- 新的架构约束写入 `MULTILANG_ARCHITECTURE.md`。
- 新的构建、测试和部署要求写入 `MULTILANG_BUILD_AND_TEST.md`。
- 阶段完成度、已知问题和决策写入 `MULTILANG_STATUS.md`。
- 只有需要保留历史上下文时才修改 archive，并在对应小文档同步当前结论。
