...
用 `React.lazy()` 按路由拆分 chunk，5 个重型详情页按需加载。

**实现方式：**
- `web/src/App.tsx` — 客户端入口，用 `React.lazy` 加载 5 个详情页
- `web/src/AppInner.tsx` — SSR 入口，同步 import（供 `ssr.tsx` 使用）
- `web/src/ssr.tsx` — 从 `AppInner.tsx` 导入，避免 SSR 遇到 lazy 组件

**拆分效果（--quick 模式）：**
- 主 bundle: 84KB → 34KB（-60%）
- `DetailPage-*.js`: 8.5KB
- `LootdropDetailPage-*.js`: 11KB
- `QuestItemGroupPage-*.js`: 11KB
- `QuestNPCDetailPage-*.js`: 9.9KB
- `DungeonModuleDetailPage-*.js`: 7.6KB
- `MapPanel-*.js`: 6.0KB（共享组件自动提取）

### 无用 JSON 文件清理 ✅

两个文件已从 `data/json/` 删除，由 `api/main.py` 的 `_deliver()` 排除：

| 文件 | 大小 | 说明 |
|------|------|------|
| `entity_index.json` | 271 KB | 管道内部使用（`quest_collector.py` 翻译兜底），前端不消费 |
| `quest_items.json` | 88 KB | 管道中间产物（生成 `quest_items_groups.json`），前端不消费 |

两个文件仍在 `api/output/json/` 中由管道生成，供内部使用，但不再交付到 `data/json/`。
