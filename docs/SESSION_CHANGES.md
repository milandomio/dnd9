...
- **Variant name i18n**: `api/src/collector.py` `_vtr` 从 `list[str]`（已解析中文名）改为 `list[dict]`（`{key, name}`），保留 translation_key 供前端多语言查询；`api/src/lootdrop_builder.py` `_get_variant_rarity` 返回 `{name, key}` 而非纯文本；前端 `VariantNameEntry` 类型 + `t(key, name)` 展示；`RARITY_COLORS` 由中文 key 改为英文 rarity tag 防止多语言不匹配；`VariantSwitch.tsx` 引入 `useLocale` 实现多语言 label
