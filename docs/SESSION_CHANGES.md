...
### merge: 将 main 已验证功能同步回 dev

- **改动原因**：`main` 已通过 cherry-pick 接收 `dev` 的两项多语言元描述功能，并额外积累多语言 SSG、DB-only 管道、Spawner、Sitemap/IndexNow 与前端质量修复；需使开发分支回到与 `main` 一致的功能基线。
- **变更文件**：`main` 相对 `dev` 的 49 个提交涉及的 80 个文件；`docs/SESSION_CHANGES.md`。
- **关键逻辑/映射关系**：合并保留 `main` 的最终树；`dev` 独有的 `7b014f99`、`06264c41` 与 `main` 的 `2de870fe`、`8a578589` 为等效元描述功能，不再重复应用。11 个冲突文件均采用已在 main 验证的最终实现，合并后暂存树与 main 完全一致。
- **验证**：合并解析后 `git diff --cached main --` 为空，`git diff --check` 与 `git diff --cached --check` 通过；`npm run format`、`npm run format:check`、`npx tsc --noEmit` 通过，`npm run lint` 为 0 error、19 条既有 warning，`npm run test:i18n` 为 23/23。
