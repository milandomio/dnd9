...
> 创建日期: 2026-07-12
> 状态: **废弃**（2026-07-26）
> 原预计收益: 4-7s（从 85s → 78s）—— 已过时，勿再引用

## 废弃原因

1. **结果级缓存已落地**：`DropRateEngine._variant_rate_cache` 按 `(ldg_id, luck_grade, full_grade, item_name)` 缓存 `compute_variant_rate` 最终 float，重复四元组直接 O(1) 返回。
2. **计划仅优化冷路径内层查找**：`grade_data` / item 后缀扫描不减外层循环（变体×组×怪×mode×楼层），相对结果缓存收益缩水为微优化（估 1–3s 级，未实测）。
3. **不改语义、不减算法层数**：维护成本高于剩余收益，不再执行。
