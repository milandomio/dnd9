...
collector.py 中 locked-merge 逻辑取 `_spawn_rate_detail` 中 same-keyword 的 locked + unlocked 之和，跨 keyword 取 max。
...
- **无需修改** collector.py/lootdrop_rates.py/前端（数据管道输出正确值后自动生效）
