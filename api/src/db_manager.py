...
class DatabaseManager:
    ...
    def import_monsters(self) -> int:
        ...
        # Add quality-suffixed entries for variants with different translation keys
        # than their base name entry (e.g. SkeletonFootmanFromFakeDeath_Unique)
        for raw_name, data_list in files.items():
            if not data_list:
                continue
            raw_stripped = raw_name.removeprefix("Id_Monster_")
            qm = _QUALITY_RE.search(raw_stripped)
            if not qm:
                continue
            quality_name = raw_stripped
            base_name = _QUALITY_RE.sub("", quality_name)
